...
#input all the value into candidate database
...
def find(data, vector):
    ...
    df['docs']   = df.groupby('author')['author'].transform('count') #count total publications
    df["related"] =  df.query("similarity >= 0.5").groupby('author')['author'].transform('count') # count total related publications
